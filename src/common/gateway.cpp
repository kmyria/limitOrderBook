...
void network_io_worker(
    std::stop_token st, struct state& gate, std::vector<int>& connections);
...
int main()
{
    ...
    struct epoll_event ev
        = { .events = EPOLLIN, .data = { .fd = gate.listener } };
    ...
    std::stop_token st, struct state& gate, std::vector<int>& connections)
{
    while (!st.stop_requested()) {
        run_server(gate.epollfd, gate.events, gate.listener, connections,
            gate.ring_buf);
    }
}
